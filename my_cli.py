...
if __name__ == "__main__":
    ...
    upload_parser.add_argument("-t", "--title", help="File path of the title of the video", required=True)
    ...
    if args.subcommand == "login":
        ...
        # START - titulo customizado vindo de um arquivo - START
        try:
            with open(args.title, 'r', encoding='utf-8') as file:
                title_content = file.read()
                tiktok.upload_video(args.users, args.video,  title_content, args.schedule, args.comment, args.duet, args.stitch, args.visibility, args.brandorganic, args.brandcontent, args.ailabel, args.proxy)
        except FileNotFoundError:
            print(f"Arquivo de título'{args.title}' não encontrado.")
            sys.exit(1)
        except Exception as e:
            print("Ocorreu um erro ao ler o arquivo de título:", e)
            sys.exit(1)
        # END   - titulo customizado vindo de um arquivo - END
